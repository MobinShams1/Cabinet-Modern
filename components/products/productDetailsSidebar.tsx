...
import { X, DollarSign, Layers, Tag, FileText, Edit, Trash2, Loader2 } from "lucide-react";
...
export default function ProductDetailSidebar({ product, isOpen, onClose, onEditClick, onProductDeleted }: ProductDetailSidebarProps) {
    ...
    <div className="w-full lg:w-[360px] bg-white border border-slate-100 rounded-xl shadow-sm flex flex-col h-full overflow-hidden transition-all duration-300">
      ...
      <div className="p-4 space-y-5 flex-1 overflow-y-auto">
        <div className="bg-slate-50/80 p-4 rounded-xl border border-slate-100/60 text-center">
          <div className="w-12 h-12 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-600 mx-auto mb-2"><Layers className="w-5 h-5" /></div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-xs p-3 bg-white border border-slate-100 rounded-lg shadow-sm">
            ...
            <span className="font-bold text-slate-800">{new Intl.NumberFormat("fa-IR").format(product.pricePerMeter)} تومان</span>
          </div>
          <div className="flex items-center justify-between text-xs p-3 bg-white border border-slate-100 rounded-lg shadow-sm">
