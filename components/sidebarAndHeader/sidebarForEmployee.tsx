...
const menuItems = [
  { id: "dashboard", label: "داشبورد", icon: <LayoutDashboard className="w-5 h-5" />, href: "/employee/dashboard" },
  { id: "orders", label: "سفارشات", icon: <ShoppingBag className="w-5 h-5" />, href: "/employee/orders" },
  { id: "products", label: "محصولات", icon: <Package className="w-5 h-5" />, href: "/employee/products" },
  { id: "inventory", label: "انبار", icon: <Warehouse className="w-5 h-5" />, href: "/employee/inventory" },
  { id: "reports", label: "گزارشات", icon: <BarChart3 className="w-5 h-5" />, href: "/employee/reports" },
];
...
export default function Sidebar({ isMobileOpen = false, onCloseMobile }: SidebarProps) {
  ...
  return (
    <>
      {/* Backdrop تاریک برای حالت کشویی موبایل */}
      {isMobileOpen && (
        <div
      ...
      <aside
        ...
        `}
      >
        {/* دکمه جمع کردن سایدبار در دسکتاپ */}
        <button
          onClick={(e) => {
            e.stopPropagation();
            toggleSidebar();
        ...
        {/* کلیک روی لوگو جهت باز و بسته شدن */}
        <div
        ...
        {/* بخش کارت پروفایل پایین سایدبار */}
